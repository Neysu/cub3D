...
void	domath(t_data *args, t_player *player_data)
{
	...
	{
		setdata(player_data, i);
		checkside(player_data);
		find_wall(args, player_data, hit);
		temp = select_text(player_data, args);
		...
		func(player_data);
		player_data->step = 1.0 * 64.0 / player_data->lineheight;
		player_data->tex_pos = (player_data->drawstart - SCREEN_HEIGHT / 2 + player_data->lineheight / 2) * player_data->step;
		draw_to_screen(i, args, player_data, temp);
		i++;
	}
}
